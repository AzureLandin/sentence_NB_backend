...
def analyze_sentence(user_id: str, sentence: str) -> dict:
    ...
    # 方法2: 尝试修复常见的 JSON 格式问题（如使用单引号）
    if analysis is None:
        try:
            # 将单引号替换为双引号（注意：这可能不完全准确，但可以尝试）
    ...
    if analysis is None:
        try:
            ...
            parse_errors.append(f"提取对象解析失败: {e}")

    if analysis is None:
        raise ValueError(
            f"无法解析 AI 返回的 JSON: {'; '.join(parse_errors)}. 原始内容: {json_str[:200]}..."
        )
...
def _build_ocr_messages(
    base64_image: str, model: str, mime: str = "image/jpeg"
) -> list:
...
def extract_sentences(
    user_id: str, base64_image: str, mime: str = "image/jpeg"
) -> list:
    """
