...
def put_ai_config():
    ...
    if "visionEndpoint" in body:
        endpoint = body["visionEndpoint"] or None
        if endpoint:
            try:
                validate_endpoint(endpoint)
            except EndpointNotAllowedError as e:
                return _err("VALIDATION_FAILED", str(e), http_status=422)
        cfg.vision_endpoint = endpoint
...
def test_ai_config():
    ...
    try:
        ...
        resp = req.post(
            api_config["endpoint"], headers=headers, json=body_payload, timeout=30
        )
