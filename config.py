...
class Config:
    ...
    # CORS 允许的 origin（逗号分隔），为空时拒绝所有跨域请求
    ...
    DEFAULT_TEXT_ENDPOINT = os.getenv(
        "DEFAULT_TEXT_ENDPOINT", "https://api.openai.com/v1/chat/completions"
    )
    DEFAULT_TEXT_MODEL = os.getenv("DEFAULT_TEXT_MODEL", "gpt-4o")

    # 平台默认视觉 AI 配置
    DEFAULT_VISION_API_KEY = os.getenv("DEFAULT_VISION_API_KEY", "")
    DEFAULT_VISION_ENDPOINT = os.getenv(
